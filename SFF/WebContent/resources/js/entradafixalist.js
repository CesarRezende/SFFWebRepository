...
var ths = $('#mainform\\:entradasFixasGrid thead tr th');
	
	var trs = $('#mainform\\:entradasFixasGrid tbody tr');
	var tds = $('#mainform\\:entradasFixasGrid tbody tr td');
	...
	$('#mainform\\:entradasFixasGrid').dataTable({
		"language": {
            "decimal": ","
            ,"thousands": "."
	...
	,"footerCallback": function ( row, data, start, end, display ) {
		...
		$( $(row).find("td").get(5)).addClass("creditrow");
...
//Cria Campos de Filtragem para Cada Coluna
    ...
    $('#mainform\\:entradasFixasGrid thead th').each( function () {
        var title = $('#mainform\\:entradasFixasGrid thead th').eq( $(this).index() ).text();
    ...
    // DataTable
    var table = $('#mainform\\:entradasFixasGrid').DataTable();
    ...
    // Apply the order
    table.columns().eq( 0 ).each( function ( colIdx ) {
    	 var title = $('#mainform\\:entradasFixasGrid thead th').eq( colIdx ).text();
         if(title != "Editar" && title != "Apagar" && title != "")
...
//Oculta Componente de Busca Padão do TabaTable
	...
	$('#mainform\\:insertButton').click(function (){ 
		window.parent.$.showModalDialog({ url: "entradafixamanager.xhtml", "height": 457, "width": 600, resizable: false, scrollable: false,  objAjax: "entradafixa|mainform\\:entradasFixasGrid", callback: loadJQueryEntradaFixa });
	});
	
	loadJQueryEntradaFixa();
});


function loadJQueryEntradaFixa(){

}

function editEntradaFixa(data) {
	...
	if (data.status === 'success') {
		...
		window.parent.$.showModalDialog({ url: "entradafixamanager.xhtml?id="+data.source.nextSibling.defaultValue, "height": 457, "width": 600, resizable: false, scrollable: false,  objAjax: "entradafixa|mainform\\:entradasFixasGrid", callback: loadJQueryEntradaFixa });
