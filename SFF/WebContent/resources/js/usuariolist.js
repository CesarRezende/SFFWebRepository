...
var ths = $('#mainform\\:usuariosGrid thead tr th');
	
	var trs = $('#mainform\\:usuariosGrid tbody tr');
	var tds = $('#mainform\\:usuariosGrid tbody tr td');
	...
	$('#mainform\\:usuariosGrid').dataTable({
		"language": {
            "decimal": ","
            ,"thousands": "."
...
//Cria Campos de Filtragem para Cada Coluna
    ...
    $('#mainform\\:usuariosGrid thead th').each( function () {
        var title = $('#mainform\\:usuariosGrid thead th').eq( $(this).index() ).text();
    ...
    // DataTable
    var table = $('#mainform\\:usuariosGrid').DataTable();
    ...
    // Apply the order
    table.columns().eq( 0 ).each( function ( colIdx ) {
    	 var title = $('#mainform\\:usuariosGrid thead th').eq( colIdx ).text();
         if(title != "Editar" && title != "Apagar" && title != "")
...
//Oculta Componente de Busca Padão do TabaTable
	...
	$('#mainform\\:insertButton').click(function (){ 
		window.parent.$.showModalDialog({ url: "usuariomanager.xhtml", "height": 320, "width": 500, resizable: false, scrollable: false,  objAjax: "usuario|mainform\\:usuariosGrid", callback: loadJQueryUsuario });
	});
...
function editUsuario(data) {
	...
	if (data.status === 'success') {
		...
		window.parent.$.showModalDialog({ url: "usuariomanager.xhtml?id="+data.source.nextSibling.defaultValue, "height": 320, "width": 500, resizable: false, scrollable: false,  objAjax: "usuario|mainform\\:usuariosGrid", callback: loadJQueryUsuario });
