...
var ths = $('#mainform\\:entradasVariaveisGrid thead tr th');
	
	var trs = $('#mainform\\:entradasVariaveisGrid tbody tr');
	var tds = $('#mainform\\:entradasVariaveisGrid tbody tr td');
	...
	$('#mainform\\:entradasVariaveisGrid').dataTable({
		"language": {
            "decimal": ","
            ,"thousands": "."
	...
	,"footerCallback": function ( row, data, start, end, display ) {
        ...
        valueCollumn = api.column( 6, { page: 'current'} ).data();
        totalValueCollumn = api.column( 7, { page: 'current'} ).data();
        totalLeftCollumn = api.column( 8, { page: 'current'} ).data();
		...
		$( $(row).find("td").get(6)).addClass("creditrow");
		$( $(row).find("td").get(7)).addClass("creditrow");
		$( $(row).find("td").get(8)).addClass("creditrow");
...
//Cria Campos de Filtragem para Cada Coluna
    ...
    $('#mainform\\:entradasVariaveisGrid thead th').each( function () {
        var title = $('#mainform\\:entradasVariaveisGrid thead th').eq( $(this).index() ).text();
    ...
    // DataTable
    var table = $('#mainform\\:entradasVariaveisGrid').DataTable();
...
//Oculta Componente de Busca Padão do TabaTable
	...
	$('#mainform\\:insertButton').click(function (){ 
		window.parent.$.showModalDialog({ url: "entradavariavelmanager.xhtml", "height": 457, "width": 600, resizable: false, scrollable: false,  objAjax: "entradavariavel|mainform\\:entradasVariaveisGrid", callback: loadJQueryEntradaVariavel });
	});
	
	loadJQueryEntradaVariavel();
});


function loadJQueryEntradaVariavel(){

}

function editEntradaVariavel(data) {
	...
	if (data.status === 'success') {
		...
		window.parent.$.showModalDialog({ url: "entradavariavelmanager.xhtml?id="+data.source.nextSibling.defaultValue, "height": 457, "width": 600, resizable: false, scrollable: false,  objAjax: "entradavariavel|mainform\\:entradasVariaveisGrid", callback: loadJQueryEntradaVariavel });
