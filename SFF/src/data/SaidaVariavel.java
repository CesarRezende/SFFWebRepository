...
/**
 * Entity implementation class for Entity: SaidaVariavels
 * 
 */
@Entity
@Audited
@Table(name = "saida_variavel")
public class SaidaVariavel extends GenericEntity implements Serializable {
	...
	@OneToMany(mappedBy = "saidaVariavel", cascade = CascadeType.ALL, orphanRemoval = true)
	private List<MovimentacaoFinanceira> movimentacoesfinanc = new ArrayList<MovimentacaoFinanceira>();

	@Transient
	private boolean newRecord = false;

	public SaidaVariavel() {
		super();
		this.setTipoGasto(new TipoGasto());
	}
	
	public static enum Status {
		ABERTO("(A) Aberto"), PAGO("(F) Pago"), PAGO_PARCIALMENTE("(A) Pago Parcial");
	...
	public static void setMovimentFinancComparator(
		...
		SaidaVariavel.movimentFinancComparator = movimentFinancComparator;
	}
